...
- Dodano `CarClickerUnlockRequirementProgress` jako view model postepu odblokowania.
- Dodano `getUnlockRequirementProgress`, ktory liczy `missingCash`, `missingTiers`, `progressRatio` i `isMet` dla wymagan `default`, `cash` i `tier`.
...
Status: pierwsza, druga, trzecia, czwarta, piata, szosta, siodma, osma, dziewiata, dziesiata, jedenasta, dwunasta, trzynasta i czternasta iteracja zrobione.
...
- `useCarClickerGame` tworzy `activeBonusViews` na podstawie aktywnych bonusow w stanie gry.
- `CarClickerStatsPanel` przyjmuje opcjonalne `activeBonusViews`.
...
- Ekran `nitro-rush.tsx` zapamietuje `lastClaimedResult` po odebraniu nagrody.
...
- Dodano domenowy typ `NitroRushTrackResolution` i wynik `NitroRushTrackResolutionOutcome`.
...
- Dodano domenowy typ `NitroRushRunProgress`.
- Dodano `getNitroRushRunProgress`, ktory zwraca liczbe przejechanych elementow, laczna liczbe elementow, procent postepu, status ukonczenia i mozliwosc odebrania nagrody.
...
- Dodano domenowe typy `NitroRushAutoRunStatus`, `NitroRushAutoRunView` i `NitroRushAutoRunStep`.
- Dodano helpery `getNitroRushAutoRunView`, `toggleNitroRushAutoRun` i `advanceNitroRushAutoRun`.
- Ekran `nitro-rush.tsx` korzysta z domenowego statusu auto-runa zamiast samodzielnie rozstrzygac, czy mozna jechac, ruszac pasem albo wlaczyc pauze.
- Timer UI wykonuje teraz jeden domenowy krok `advanceNitroRushAutoRun`, ktory zwraca rezultat, nowy postep i informacje, czy run ma jechac dalej.
- Dodano widoczny status runa: `Gotowy`, `Jedzie`, `Pauza` albo `Meta`.
